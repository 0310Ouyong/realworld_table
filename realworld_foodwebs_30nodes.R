...
for(i in 1:290){
  if(length(unique(c(filter(Select_Brose_data,foodweb.name%in%foodweb.name1[i])$res.taxonomy,filter(Select_Brose_data,foodweb.name%in%foodweb.name1[i])$con.taxonomy)))>=30){
    foodweb_30nodes1 <- rbind(foodweb_30nodes1,filter(Select_Brose_data,foodweb.name%in%foodweb.name1[i]))
  }
}

foodweb.name2 <- unique(foodweb_30nodes1$foodweb.name) #191
...
foodweb.name3 <- unique(foodweb_30nodes2$foodweb.name) #179
...
foodweb.name4 <- unique(foodweb_30nodes3$foodweb.name) #179
...
foodweb.name5 <- unique(foodweb_30nodes4$foodweb.name) #108
...
length(unique(foodweb_30nodes5$foodweb.name)) #174

realworld_foodwebs_30nodes <- unique(foodweb_30nodes5$foodweb.name)
save(realworld_foodwebs_30nodes,file = 'realworld_foodwebs_30nodes.RData')
