...
#remove cannibalistic links
cannibalistic_links <- which(Select_Brose_data$con.taxonomy == Select_Brose_data$res.taxonomy)
all_number <- c(1:45859)
select_number <- setdiff(all_number,cannibalistic_links)
Select_Brose_data <- Select_Brose_data[select_number,]
...
save(rep_webs,file = 'remove_cannibalistic_Empiwebs_30nodes.RData')
